...
import { Mic, BrainCircuit, Clock, AlertTriangle, Zap, Search, MessageSquare, Target } from "lucide-react";
...
const FeatureCard = ({ title, description, icon: Icon, delay }: { title: string, description: string, icon: any, delay: number }) => (
    <motion.div
        ...
        className="group relative overflow-hidden p-8 rounded-[2rem] bg-[#0d1117] border border-white/5 hover:border-primary/20 transition-all duration-300"
    >
        ...
        <p className="text-sm font-medium text-white/40 leading-relaxed group-hover:text-white/60 transition-colors">
            {description}
        </p>
    </motion.div>
);
...
export function FeatureSection() {
    return (
        <section id="features" className="py-40 px-4 bg-black relative overflow-hidden">
            {/* Background Effects */}
            ...
            <div className="max-w-7xl mx-auto relative z-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 px-4 relative z-10 mb-4">
                    <FeatureCard
                        title="Voice Note Intelligence"
                        description="Audnix doesn't just read text. It listens to Voice Notes, extracts sentiment, and adapts its reply tone instantly."
                        icon={Mic}
                        delay={0}
                    />
                    <FeatureCard
                        title="Real-Time Intent Check"
                        description="Every reply is analyzed against 110+ objection scenarios and buying signals before a single word is sent back."
                        icon={BrainCircuit}
                        delay={0.1}
                    />
                    <FeatureCard
                        title="Predictive Timing"
                        description="Uses 'Human-Like Delays' (2-8 mins) and checks user activity to respond exactly when they are most likely to convert."
                        icon={Clock}
                        delay={0.2}
                    />
                    <FeatureCard
                        title="Churn & Drop-off Risk"
                        description="Identifies leads losing interest and automatically deploys a 'Re-Engagement Protocol' to recover the sale."
                        icon={AlertTriangle}
                        delay={0.3}
                    />
                </div>

                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 px-4 relative z-10">
                    <div className="lg:col-span-2 group relative overflow-hidden rounded-[2.5rem] bg-[#0d1117] border border-white/5 p-8 md:p-12">
                        ...
                        <div className="relative z-10">
                            <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center border border-white/10 mb-8 group-hover:scale-110 transition-transform duration-500">
                    ...
                    <div className="group relative overflow-hidden rounded-[2.5rem] bg-[#0d1117] border border-white/5 p-8 md:p-12">
                        <div className="relative z-10 h-full flex flex-col justify-between">
                            <div>
                                <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center border border-white/10 mb-8 group-hover:scale-110 transition-transform duration-500">
                                    <Search className="w-6 h-6 text-white" />
                                </div>
                                <h3 className="text-3xl font-black mb-4 tracking-tight">Deep Lead Insights</h3>
