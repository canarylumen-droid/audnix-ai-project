                        ...
                        {FEATURES.map((feature, i) => (
                            <motion.button
                                ...
                                className={`w-full text-left p-8 rounded-[2rem] border transition-all duration-500 group relative overflow-hidden ${activeFeature.id === feature.id
                                        ? "bg-primary/10 border-primary/30 shadow-[0_0_30px_rgba(var(--primary),0.1)]"
                                        : "bg-white/[0.02] border-white/5 hover:border-white/20"
                                    }`}
                            >
                                <div className="flex items-center gap-6">
                                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-500 ${activeFeature.id === feature.id ? "bg-primary text-black" : "bg-white/5 text-white/40 group-hover:text-white"
                                        }`}>
                    ...
                    <div className="lg:sticky lg:top-32">
                        <AnimatePresence mode="wait">
                            <motion.div
                                ...
                                className="p-12 rounded-[3rem] bg-black/80 backdrop-blur-3xl border border-white/10 relative overflow-hidden h-full"
                            >
                                {/* Decorative Glow */}
                                ...
                                <div className="space-y-12">
                                    <div className="flex items-center gap-6">
                                        <div className="w-20 h-20 rounded-[1.5rem] bg-primary flex items-center justify-center text-black shadow-[0_0_40px_rgba(var(--primary),0.5)]">
                                            <activeFeature.icon className="w-10 h-10" />
                                        </div>
                                    ...
                                    <div className="space-y-6">
                                        <h4 className="text-[10px] font-black uppercase tracking-[0.4em] text-primary">Technical Overview</h4>
                                    ...
                                    <div className="grid md:grid-cols-2 gap-10">
                                        <div className="space-y-6">
                                            <h4 className="text-[10px] font-black uppercase tracking-[0.4em] text-white/30">Business Outcomes</h4>
                                            <ul className="space-y-4">
                                                {activeFeature.outcomes.map((item, i) => (
                                                    <li key={i} className="flex items-center gap-3 text-sm font-bold text-white border-l-4 border-primary pl-4 py-1">
                                                        {item}
                                                    </li>
                                                ))}
                                            </ul>
                                        ...
                                        <div className="space-y-6">
                                            <h4 className="text-[10px] font-black uppercase tracking-[0.4em] text-white/30">System Specs</h4>
                                            <ul className="space-y-4">
                                                {activeFeature.details.map((item, i) => (
                                                    <li key={i} className="flex items-center gap-3 text-sm font-bold text-white/40 leading-loose">
