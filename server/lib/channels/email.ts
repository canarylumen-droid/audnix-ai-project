...
export async function sendEmail(
  ...
  if (!emailIntegration) {
    console.error('sendEmail: No email integration found for user', userId);
    throw new Error('Email not connected. Please connect your business email in Settings.');
  }
  ...
  const { decrypt } = await import('../crypto/encryption.js');
  if (!emailIntegration.encryptedMeta) {
    throw new Error('Email integration credentials missing');
  }

  let credentials: EmailCredentials;
  try {
    const decrypted = await decrypt(emailIntegration.encryptedMeta);
    credentials = JSON.parse(decrypted);
  } catch (err) {
    console.error('Failed to decrypt email credentials:', err);
    throw new Error('Invalid email configuration');
  }

  if (!credentials || !credentials.accessToken || !credentials.email) {
    throw new Error('Invalid email credentials structure');
  }
...
export async function getEmailInbox(
  ...
  try {
    ...
    console.error('Failed to decrypt email credentials for inbox:', err);
    throw new Error('Invalid email configuration');
  }
