...
router.get("/deals/analytics", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const allDeals = await storage.getDeals({ userId });
    const convertedDeals = allDeals.filter((d: any) => d.status === 'converted');
    ...
    const prevWeekDeals = convertedDeals.filter((d: any) => {
      ...
      return date >= prevWeekStart && date < startOfWeek;
    });

    const previousWeekRevenue = prevWeekDeals.reduce((sum: number, d: any) => sum + (d.value || 0), 0);
    const previousMonthRevenue = convertedDeals
      .filter((d: any) => {
        const date = new Date(d.convertedAt);
        return date >= prevMonthStart && date < startOfMonth;
      })
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      const dayRevenue = convertedDeals
        .filter((d: any) => {
          const convertedDate = new Date(d.convertedAt);
